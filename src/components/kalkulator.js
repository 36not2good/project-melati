...
const Kalkulator = () => {
  ...
  const calculateResult = () => {
    try {
      let expression;
      if (operator === '^') {
        expression = Math.pow(parseFloat(firstNumber), parseFloat(secondNumber));
      } else if (operator === '%') {
        expression = parseFloat(firstNumber) % parseFloat(secondNumber);
      } else {
        expression = eval(`${firstNumber} ${operator} ${secondNumber}`);
      }
